...
if($terjadi == "ya"){
	...
	if($tanggal_infeksi <= $tanggal_lepas && $tanggal_infeksi >= $tanggal_pasang){
		...
		window.location.href='../create-surveilans-phlebitis.php?id=$id_surv' </script>";
	}
...
$id_k = mysqli_fetch_array(mysqli_query($conn, "SELECT id_pemakaian_ruangan FROM surv_phlebitis WHERE id_surv = '$id_surv'"));

if($tanggal_pasang <= $today && $tanggal_lepas <= $today){

	if($tanggal_pasang <= $tanggal_lepas){

		$sql = mysqli_query($conn, "INSERT INTO surv_gejala_phlebitis (id_surv, lokasi_pasang, tanggal_pasang, bundles_kebersihan, bundles_lepas_bykeluhan, bundles_cek_balutan, bundles_lepas_bywaktu, bundles_cek_tempat, tanggal_lepas, total_hari, terjadi_infeksi, tanggal_infeksi, tanda_infeksi) values ('$id_surv', '$lokasi', '$pasangDate', '$bundles1', '$bundles2', '$bundles3', '$bundles4', '$bundles5', '$lepasDate', '$total_hari', '$terjadi', '$infeksiDate', '$selected_tanda')") && mysqli_query($conn, "UPDATE surv_phlebitis SET terjadi_infeksi = '$terjadi', tanggal_infeksi = '$infeksiDate' WHERE id_surv = $id_surv");
